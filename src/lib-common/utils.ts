...
export class BytesFIFOBuffer {
	...
	/**
	 * Creates FIFO buffer for bytes that works both with Buffer's and
	 * Uint8Array's.
	 * @param useBuffer is an optional flag that forces use of Buffer's.
	 * Default value false indicates use of Uint8Array's.
	 */
	constructor(
	...
	private extractSomeBytesFrom(extractLen: number): Uint8Array|undefined {
		...
		const extract = (this.useBuffer ?
			Buffer.allocUnsafe(extractLen) :
			new Uint8Array(extractLen));
		let offset = 0;
